...
def ppo(
    ...
    *,
    ...
    critic_model = AutoPatchForReward.from_arguments(config.critic)
    ...
    critic_optimizer = AdamW(
        critic_params,
        lr=config.algorithm.update_critic.optim.lr,
        weight_decay=config.algorithm.update_critic.optim.weight_decay,
    )
    ...
    for step in range(start_step, total_steps):
        ...
        if step < config.algorithm.update_actor.warmup_steps:
        ...
        old_values = compute_values_loop(
            critic_model,
            infer_dataset,
            config.algorithm.update_critic.dataloader.micro_batch_size,
            critic_model.sequence_parallel_mesh,
        )
        ...
        assert len(old_logprobs) == len(ref_logprobs) == len(old_values) == len(scores)
        ...
        for _ in range(config.algorithm.update_critic.epochs):
            update_critic_log = update_critic_loop(
                critic=critic_model,
                loss_fn=critic_loss_fn,
                optimizer=critic_optimizer,
                max_grad_norm=config.algorithm.update_critic.optim.max_grad_norm,
                dataset=trajectory_dataset,
                micro_batch_size=config.algorithm.update_critic.dataloader.micro_batch_size,
                global_batch_size=config.algorithm.update_critic.dataloader.global_batch_size,
                optimizer_step_times=1,
                dp_mesh=actor_dp_mesh,
                sp_mesh=actor_sp_mesh,
            )
        ...
        log_dict["actor_lr"] = (
            config.algorithm.update_actor.optim.lr if update_actor else 0.0
        )
        log_dict["critic_lr"] = config.algorithm.update_critic.optim.lr

        log_dict.update(update_actor_log)
        log_dict.update(update_critic_log)
